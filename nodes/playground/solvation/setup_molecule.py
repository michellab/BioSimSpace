...
node.addInput("boxsize", BSS.Gateway.Length(help="The extent of the water shell along each axis around the solute.",
                                            unit="angstrom"))
...
if isVerbose:
    ...
    node.getInput("water"), str(node.getInput("boxsize")), node.getInput('ion_conc')))
system_solvated = BSS.Solvent.solvate(node.getInput("water"), molecule=molecule, box=3 * [node.getInput("boxsize")],
