...
node.addInput(
    "molecule", BSS.Gateway.File(help="A molecular input file, e.g. a PDB file.")
)
node.addInput(
    "forcefield",
    BSS.Gateway.String(
...
node.addInput(
    "parametrisation_base",
    BSS.Gateway.String(help="The root name of the output files."),
)
node.addOutput(
    "parametrisation_out",
    BSS.Gateway.FileSet(
...
node.addInput(
    "minimisation_base",
    BSS.Gateway.String(help="The base name for the minimised system files"),
)
node.addOutput(
    "minimisation_out",
    BSS.Gateway.FileSet(
...
node.setOutput(
    ...
    BSS.IO.saveMolecules(
        node.getInput("parametrisation_base"), molecule, ["prm7", "rst7"]
    ),
)

if isVerbose:
    print(
        "Solvating system in %s water in a box of size %s with an ion concentrations of %f."
        % (
            node.getInput("water"),
            str(node.getInput("boxsize")),
            node.getInput("ion_conc"),
        )
    )
system_solvated = BSS.Solvent.solvate(
    node.getInput("water"),
    molecule=molecule,
    box=3 * [node.getInput("boxsize")],
...
node.setOutput(
    "minimisation_out",
    BSS.IO.saveMolecules(
        node.getInput("minimisation_base"),
