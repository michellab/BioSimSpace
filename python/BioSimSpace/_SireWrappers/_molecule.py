...
class Molecule():
    ...
    def _toAmberWater(self):
        """Internal function to rename a water molecule to match the template
           expected by AMBER.
        """
        self._toWaterModel("WAT", "H1", "H2", "O", "EPW")

    def _toGromacsWater(self):
        """Internal function to rename a water molecule to match the template
           expected by GROMACS.
        """
        self._toWaterModel("SOL", "HW1", "HW2", "OW", "MW")

    def _toWaterModel(self, residue, hydrogen0, hydrogen1, oxygen, dummy):
        """Internal function to rename water molecule to match the template
           of different water models.
           ...
           hydrogen0 : str
               The name of the first hydrogen atom.

           hydrogen1 : str
               The name of the second hydrogen atom.

           oxygen : str
               The name of the oxygen atom.
        ...
        if type(residue) is not str:
            raise TypeError("'hydrogen0' must be of type 'str'.")

        if type(hydrogen0) is not str:
            raise TypeError("'hydrogen0' must be of type 'str'.")

        if type(hydrogen1) is not str:
            raise TypeError("'hydrogen1' must be of type 'str'.")

        if type(oxygen) is not str:
            raise TypeError("'oxygen' must be of type 'str'.")

        if type(dummy) is not str:
            raise TypeError("'dummy' must be of type 'str'.")

        # Make the molecule editable.
        molecule = self._sire_molecule.edit()

        # Update the molecule with the new residue name.
        molecule = molecule.residue(_SireMol.ResIdx(0))    \
                        .rename(_SireMol.ResName(residue)) \
                        .molecule()
        ...
        for atom in molecule.atoms():
            try:
                ...
                if atom.property("element") == _SireMol.Element("H"):
                    if hydrogen_idx == 0:
                        molecule = molecule.atom(atom.number())                  \
                                           .rename(_SireMol.AtomName(hydrogen0)) \
                                           .molecule()
                    else:
                        molecule = molecule.atom(atom.number())                  \
                                           .rename(_SireMol.AtomName(hydrogen1)) \
                                           .molecule()
                    hydrogen_idx += 1
                ...
                # Oxygen.
                    ...
                    molecule = molecule.atom(atom.number())               \
                                       .rename(_SireMol.AtomName(oxygen)) \
                                       .molecule()

                # Dummy.
                elif atom.property("element") == _SireMol.Element("X"):
                    molecule = molecule.atom(atom.number())              \
                                       .rename(_SireMol.AtomName(dummy)) \
                                       .molecule()
            ...
            # Otherwise, try to infer the element from the atom name.
                ...
                # Dummy.
                elif element == _SireMol.Element("X"):
                    molecule = molecule.atom(atom.number())              \
                                       .rename(_SireMol.AtomName(dummy)) \
                                       .molecule()

        # Commit the changes.
        self._sire_molecule = molecule.commit()
