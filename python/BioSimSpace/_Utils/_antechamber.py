...
if _amber_home is not None:
    if _os.path.isfile("%s/bin/parmchk" % _amber_home):
        _parmchk_exe = "%s/bin/parmchk" % _amber_home

if _parmchk_exe is None:
    # Search Sire bin directory.
    _bin_dir = _Sire.Base.getBinDir()
    _parmchk_exe = "%s/parmchk" % _bin_dir
...
class Antechamber():
    ...
    def parameterise(molecule, forcefield, frcmod=None, work_dir=None, verbose=False):
        ...
        if _os.path.isfile("antechamber.mol2"):
            ...
            # Read the output mol2 file and extract the molecule from the system.
            new_mol = _Sire.IO.MoleculeParser.read("antechamber.mol2")[_Sire.Mol.MolIdx(0)]
            ...
            if _os.path.isfile("antechamber.frcmod"):
                ...
                return _Tleap.parameterise(new_mol, forcefield, work_dir=work_dir,
