...
class FreeEnergy():
    ...
    def _initialise_runner(self, system0, system1):
        ...
        # equilibrate. Here we use AMBER directly.

        # Leg 0
        if len(waters0) > 0:
            # Create a copy of the solvated system for leg 0.
            system = system0.copy()

            # Extract the perturbed molecule, converting it to a regular molecule
            # at the lambda = 0 state.
            regular_mol = system.getPerturbableMolecules()[0]._toRegularMolecule()

            # Replace the perturbed molecule with the regular one.
            system.updateMolecules(regular_mol)
            ...
            # Exit if the minimisation failed.
            if process.isError():
                raise RuntimeError("Minimisation of the first leg lambda = 0 state failed!")

            # Create the equilibration protocol.
            protocol = _Protocol.Equilibration(temperature_start=0*_Units.Temperature.kelvin,
                                               temperature_end=self._protocol.getTemperature(),
            ...
            # Exit if the equilibration failed.
            if process.isError():
                raise RuntimeError("Equilibration of first leg lambda = 0 state failed!")

            # Copy the equilibrated coordinates back into the original system.
            system0._updateCoordinates(process.getSystem())

        # lambda = 1
        if len(waters0) > 0:
            # Create a copy of the solvated system for leg 1.
            system = system1.copy()
            ...
            # Exit if the minimisation failed.
            if process.isError():
                raise RuntimeError("Minimisation of the second leg lambda = 0 state failed!")
            ...
            # Exit if the equilibration failed.
            if process.isError():
                raise RuntimeError("Equilibration of the second leg lambda = 0 state failed!")

            # Copy the equilibrated coordinates back into the original system.
            system1._updateCoordinates(process.getSystem())
