...
if [ "$(uname)" == "Linux" ]; then
    BINARY=biosimspace_${TAG}_linux.run
else
    BINARY=biosimspace_${TAG}_osx.run
fi

# Copy the latest devel binary to the release version.
cp $HOME/biosimspace_devel_latest_*.run $HOME/$BINARY

# Deploy the binary.
$HOME/sire.app/bin/python $SRC_DIR/docker/deploy-devel/deploy.py $HOME/$BINARY
